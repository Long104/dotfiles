...
function todo
    if test -z "$TMUX"
        if tmux has-session -t "todo📝" 2>/dev/null
            ...
            # Split vertically (pane 0: top, pane 1: bottom)
            tmux split-window -v -c ~/satori/note/todo -t "todo📝:1.1"
            
            # Top pane configuration
            tmux send-keys -t "todo📝:1.1" "env NVIM_APPNAME=Shoshinvim nvim TODO.md" Enter
            tmux send-keys -t "todo📝:1.1" " "hc
            
            # Bottom pane configuration
            tmux send-keys -t "todo📝:1.2" "env NVIM_APPNAME=Shoshinvim nvim TODO.md -c ':ObsidianDailies'" Enter
            sleep 1
            tmux send -t "todo📝:1.2" "today"
            sleep 1
            tmux send-keys -t "todo📝:1.2" Enter
            tmux send-keys -t "todo📝:1.2" " "hc
        ...
        if tmux has-session -t "todo📝" 2>/dev/null
            ...
            tmux new-session -d -s "todo📝"
            tmux send-keys -t "todo📝" "cd ~/satori/note/todo" Enter
            tmux split-window -v -c ~/satori/note/todo -t "todo📝:1.1"
            tmux send-keys -t "todo📝:1.1" "env NVIM_APPNAME=Shoshinvim nvim TODO.md" Enter
            tmux send-keys -t "todo📝:1.1" " "hc
            tmux send-keys -t "todo📝:1.2" "env NVIM_APPNAME=Shoshinvim nvim TODO.md -c ':ObsidianDailies'" Enter
